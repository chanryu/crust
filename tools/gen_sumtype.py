...
def cpp_ident(name: str) -> str:
    if not isinstance(name, str):
        raise ValueError(
            f"Identifier must be a string, got {type(name).__name__}")
...
def normalize_field(field: Any, ctx: str) -> Dict[str, Any]:
    ...
    if isinstance(field, list):
        if len(field) not in (2, 3):
            raise ValueError(
                f"{ctx}: field array must be [name,type] or [name,type,default]")
    ...
    if isinstance(case, dict):
        ...
        if len(case) == 1:
            ...
            if not isinstance(v, list):
                raise ValueError(
                    f"{ctx}: shorthand case value must be a list of fields")
            if v:
                out["fields"] = [normalize_field(
                    f, f"{ctx}['{k}']") for f in v]
            return out
    ...
    for case in cases:
        ...
        for f in fields:
            ...
            if fname in seen_fields:
                raise ValueError(
                    f"Case '{cname}': duplicate field name '{fname}'")
            seen_fields.add(fname)
            if not isinstance(f["type"], str) or not f["type"].strip():
                raise ValueError(
                    f"Case '{cname}': field 'type' must be a non-empty string")
            if "default" in f and not isinstance(f["default"], str):
                raise ValueError(
                    f"Case '{cname}': field 'default' must be a string (raw C++)")
...
def emit_case(case: Dict[str, Any], enum_name: str, indent: str) -> str:
    ...
    if fields:
        lines.append(
            f"{indent}    constexpr {cname}({ctor_param_list(fields)}) : {ctor_init_list(fields)} {{}}")
...
def emit_variant(enum_name: str, cases: List[Dict[str, Any]]) -> str:
    ...
    lines: List[str] = [
        ...
        f"    {enum_name}& operator=(const {enum_name}&) = default;",
        f"    {enum_name}& operator=({enum_name}&&) = default;",
        "",
    ...
    lines.append("")
    lines.append(
        f"    // Convenience constructors: {enum_name} v = {enum_name}::Case{{...}};")
    for c in cases:
        cname = cpp_ident(c["name"])
        lines.append(
            f"    constexpr {enum_name}({cname} v) : Base(std::move(v)) {{}}")
